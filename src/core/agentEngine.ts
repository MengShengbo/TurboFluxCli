...
export class AgentEngine {
  ...
  async runFastContextObjective(objective: string): Promise<FastContextScanResult | null> {
    ...
    if (this.fastContextRunPromise && this.fastContextObjective === nextObjective) {
      return this.fastContextRunPromise
    }
    this.setFastContextObjective(nextObjective)
    const promise = this.maybeRunFastContextScan()
  ...
  async run(userMessage: string, options?: { reuseLastUserTurn?: boolean }): Promise<AgentTurn[]> {
    ...
    if (this.fastContextObjective && !this.fastContextPack) {
      await this.runFastContextObjective(this.fastContextObjective || userMessage)
    }
...
5. After fixing the approach, re-attempt with corrected parameters
  ...
  private async maybeRunFastContextScan(userMessage = ''): Promise<FastContextScanResult | null> {
    if (!this.config.workspacePath || !this.fastContextObjective) return null
  ...
  private async maybeFetchCodemapSummary(strategy?: TurnStrategy | null): Promise<void> {
    ...
    try {
      ...
      }) as { success: boolean; map?: CodeMapNode; relatedPaths?: string[] }
      if (!response.success || !response.map) return
      const related = response.relatedPaths?.length ? `\nRelated paths:\n${response.relatedPaths.map((p: string) => `- ${p}`).join('\n')}` : ''
      this.codemapSummary = `${this.formatCodeMap(response.map)}${related}`.slice(0, 5000)
    } catch (err) {
  ...
  private async dispatchTool(name: string, args: Record<string, unknown>): Promise<string> {
    ...
    switch (name) {
      ...
      case 'get_codemap': {
        ...
        if (!response.data?.map) return 'No codemap found'
        const related = response.data?.relatedPaths?.length ? `\n\nRelated paths:\n${response.data.relatedPaths.map((p: string) => `- ${p}`).join('\n')}` : ''
        return `${this.formatCodeMap(response.data.map)}${related}`
      }
